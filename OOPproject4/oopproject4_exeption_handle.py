...
#matplotlib 이용해 이미지 파일 읽어 시각화
img_ori = cv2.imread('test_image/car3.png')
...
###5th image, Selecting Candidates (1)chars###
...
result_idx = find_chars(possible_contours)   #find_chars.py 에 함수를 작성했음
...
###7th image, Crop and revise warped using tesseract###
...
###8th image, Getting another candidate ###
...
#반복문 이용하는 걸로 바꾸기, chars가 한정된 길이로만 접근하도록 변경
correct = 0
for x in range(len(chars)):
    if x == 2 or x == 3:
        if chars[x].isdigit() or isHangeul(chars[x]):
            correct += 1
    else:
        if chars[x].isdigit():
            correct += 1
